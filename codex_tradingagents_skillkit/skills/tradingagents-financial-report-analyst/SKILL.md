...
18. For each ASX sector metric direction, preserve an auditable record with `metric_name`, `extracted_value_or_phrase`, `clean_metric_value`, `value_unit`, `value_context`, `metric_value_status`, `association_score`, `association_reason`, `period_reference`, `comparison_reference`, `supporting_sentence`, `comparison_basis`, `direction`, `confidence`, `confidence_reason`, and `evidence_id`; include table context fields such as `table_title`, `row_label`, `column_label`, `source_page`, `current_period_value`, `prior_period_value`, `variance_value`, and `variance_percent` when available. Do not rely only on metric presence, isolated keywords, or nearest-number matching.
19. Downgrade navigation/page-list snippets to low-confidence or context-only evidence. Do not mark reserves/resources or commodity exposure supportive unless the extracted phrase contains a real metric or clearly relevant exposure statement.
20. For ASX metrics, use metric-specific extraction profiles and label-value association scoring. Populate `clean_metric_value` only when the value is semantically attached to the metric label with high association confidence; otherwise keep the clean value unavailable and set `metric_value_status` to `direction_extracted`, `metric_mentioned_only`, `context_only`, `table_row_unparsed`, or `unavailable`. If a dense table-like row contains many numbers and multiple financial row labels, do not accept nearest-number association unless row/column structure is parsed.
21. For ASX review-ready runs, core sector metrics are a minimum coverage standard. A core metric must be either `value_extracted` with auditable value mapping or explicitly unavailable because the reviewed source genuinely lacks it; `metric_mentioned_only`, `direction_extracted`, `table_row_unparsed`, and low-confidence `context_only` are evidence gaps that block normal review-ready completion.
