...
18. For each ASX sector metric direction, preserve an auditable record with `metric_name`, `extracted_value_or_phrase`, `clean_metric_value`, `value_unit`, `value_context`, `period_reference`, `comparison_reference`, `supporting_sentence`, `comparison_basis`, `direction`, `confidence`, `confidence_reason`, and `evidence_id`. Do not rely only on metric presence or isolated keywords.
