...
18. For each ASX sector metric direction, preserve an auditable record with `metric_name`, `extracted_value_or_phrase`, `clean_metric_value_if_available`, `supporting_sentence`, `comparison_basis`, `direction`, `confidence`, `confidence_reason`, and `evidence_id`. Do not rely only on metric presence or isolated keywords.
